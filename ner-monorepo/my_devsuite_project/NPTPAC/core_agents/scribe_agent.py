...
# Add this function definition within your scribe_agent.py, e.g., before the ScribeAgent class or main()

def _parse_lily_interaction_log_to_json(input_log_path: Path, output_json_path: Path) -> Tuple[bool, str, int]:
    """
    Parses a raw Lily interaction log file (text) into a structured JSON file.
    Each turn is expected to be prefixed by "Architect:" or "Lily:".
    Timestamps are not assumed in this basic parser but can be added if logs have them.
    """
    local_logger = logging.getLogger(f"{APP_NAME}.LilyLogParser")
    local_logger.info(f"Parsing Lily log: {input_log_path}")
    ...
    try:
        with open(input_log_path, "r", encoding="utf-8") as f:
            ...
            current_text_lines = []
            # Regex to find speaker lines, case-insensitive, allowing for space variations
            speaker_regex = re.compile(r"^\s*(Architect|Lily|Rick|Core Team Member [A-Za-z ]+)\s*:\s*(.*)", re.IGNORECASE)
                ...
                if match:
                    ...
                    if current_speaker and current_text_lines:
                        turns.append({
                            ...
                            "timestamp": None, # Placeholder, implement if logs have timestamps
                    ...
                    # If Rick or Core Team Member, normalize speaker for consistency if needed
                    if "Rick" in current_speaker: current_speaker = "Rick" 
                    # elif "Core Team Member" in current_speaker: current_speaker = "Core Team Member" # Or keep specific name
                    
                    initial_text = match.group(2).strip()
                    if initial_text: # Only add if there's text after the colon
                        current_text_lines.append(initial_text + "\n") # Preserve newlines for multi-line
                elif current_speaker: # Line doesn't start with a speaker, so it's a continuation
                    current_text_lines.append(line_content) # Keep original newlines
        ...
        output_data = {
            "log_source_file": str(input_log_path.name),
            "parsing_timestamp_utc": datetime.now(timezone.utc).isoformat(),
            "parser_version": "ScribeAgent.LilyLogParser_v1.1", # Version your parser
            "turns": turns
        }
        ...
        msg = f"Input log file not found during parsing: {input_log_path}"
        local_logger.error(msg)
        return False, msg, 0
    except Exception as e:
        msg = f"Error parsing log file '{input_log_path.name}': {type(e).__name__} - {e}"
        local_logger.error(msg, exc_info=True)
        return False, msg, 0
...
def setup_arg_parser() -> argparse.ArgumentParser:
    ...
    core_input_group.add_argument(
        '--task-mode',
        type=str,
        default="validation_gauntlet", # Existing default mode
        choices=["validation_gauntlet", "lily_log_parse"],
        help="Operational mode for ScribeAgent."
    )
    core_input_group.add_argument(
        '--input-log-file', # Used by lily_log_parse mode
        type=Path,
        default=None,
        help="Path to the raw Lily interaction log file to be parsed."
    )
    core_input_group.add_argument(
        '--output-structured-log-file', # Used by lily_log_parse mode
        type=Path,
        default=None,
        help="Path where the structured JSON output of log parsing should be saved."
    )
    # Optional:
    core_input_group.add_argument(
        '--ner-task-definition', # If Scribe needs to load more rules from a NER file for this mode
       type=str,
       default=None,
        help="NER path to a detailed task definition for the current task-mode (optional)."
    )
    core_input_group.add_argument(
        '--target-dir',
...
def main(cli_args: Optional[Sequence[str]] = None) -> int:
    ...
    # --- Handle Lily Log Parsing Mode ---
    if parsed_args.task_mode == "lily_log_parse":
        main_logger.info("ScribeAgent initiated in 'lily_log_parse' mode.")
        if not parsed_args.input_log_file or not parsed_args.output_structured_log_file:
            err_msg = "--input-log-file and --output-structured-log-file are required for lily_log_parse mode."
            main_logger.critical(err_msg)
            # Print JSON error to stdout as expected by BaseAgentRunner/pac_cli
            print(json.dumps({
                "status": "failure",
                "message": err_msg,
                "parsed_log_output_path": None,
                "parsed_turn_count": 0
            }))
            return 1 # Exit with error

        main_logger.info(f"Input log for Lily: {parsed_args.input_log_file}")
        main_logger.info(f"Designated output for structured log: {parsed_args.output_structured_log_file}")

        try:
            # Call the new parsing function (define this function within scribe_agent.py)
            success, message, turns_count = _parse_lily_interaction_log_to_json(
                Path(parsed_args.input_log_file),
                Path(parsed_args.output_structured_log_file)
            )
            
            stdout_payload = {
                "status": "success" if success else "failure",
                "message": message,
                "parsed_log_output_path": str(Path(parsed_args.output_structured_log_file).resolve()) if success else None,
                "parsed_turn_count": turns_count if success else 0
            }
            print(json.dumps(stdout_payload)) # CRITICAL: Output JSON to stdout
            main_logger.info(f"Lily log parsing finished. Status: {stdout_payload['status']}. Message: {stdout_payload['message']}")
            return 0 if success else 1
            
        except Exception as e_parse_lily:
            main_logger.error(f"Critical error during Lily log parsing execution: {e_parse_lily}", exc_info=True)
            stdout_payload = {
                "status": "failure",
                "message": f"Exception in lily_log_parse mode: {type(e_parse_lily).__name__} - {e_parse_lily}",
                "parsed_log_output_path": None,
                "parsed_turn_count": 0
            }
            print(json.dumps(stdout_payload))
            return 1 # Failure
    # --- End of Lily Log Parsing Mode Handling ---
