...
class CommandLineTool(object):
    def __init__(self):
        ...
        self.parser.add_argument(
            "aesmode", help="new AES mode [one of: %s]" % ", ".join(self.aesmodes)
        )
        self.parser.add_argument("infile", help="Keyring file to convert", nargs="?")
        self.parser.add_argument(
            "outfile", help="New keyring file [default: infile.pid]", nargs="?"
        )
        self.parser.add_argument(
            "-v", "--verbose", help="verbose mode (cumulative)", action="count"
        )
        self.parser.add_argument(
            "-k", "--keep", help="keep old password", action="store_true"
        )
    ...
    def setup_logging(self, verbose):
        for idx, loglevel in enumerate(range(logging.WARNING, logging.NOTSET, -10)):
            if idx == verbose:
                break
        logging.basicConfig(
            level=loglevel,
