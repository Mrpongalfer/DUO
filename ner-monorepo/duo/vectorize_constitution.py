...
NER_RULES_DIR = (
    NER_PATH_FROM_ENV
    if NER_PATH_FROM_ENV
    else (
        os.getenv(
            "NER_ABSOLUTE_PATH", "/home/pong/Projects/games/NPTPAC/ner_repository"
        )
        if os.getenv("NER_ABSOLUTE_PATH")
        else "Rules"
    )
)
...
class ConstitutionVectorizer:
    ...
    def __init__(self, rules_dir: str, output_dir: str):
        ...
        )  # parents=True in case output_dir is nested
        ...
        try:
            with httpx.Client(timeout=60.0) as client:
                response = client.post(
                    OLLAMA_API_URL, json={"model": EMBEDDING_MODEL, "prompt": text}
                )
            ...
            if embedding_data:
                ...
                print(
                    "Warning: API response did not contain an embedding for a document.",
                    file=sys.stderr,
                )
                return None
            ...
            print(
                f"Please ensure your Ollama server is running and the model '{EMBEDDING_MODEL}' is available.",
                file=sys.stderr,
            )
            return None
        except (json.JSONDecodeError, KeyError):
            print(
                "Error: Failed to parse JSON response from Ollama API.",
                file=sys.stderr,
            )
            return None
    ...
    def vectorize_all_documents(self):
        ...
        if not self.rules_dir.is_dir():
            print(
                f"Error: Rules directory not found at '{self.rules_dir}'",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if not found_files:
            print(
                f"Warning: No markdown files found in '{self.rules_dir}' or its subdirectories. No vectors will be generated.",
                file=sys.stderr,
            )
            return
        ...
        for doc_path in found_files:
            print(
                f"\nProcessing '{doc_path.relative_to(self.rules_dir.parent)}'..."
            ...
            try:
                ...
                if embedding is not None:
                    ...
                    if relative_path.parent != Path("."):  # If it's in a subdirectory
                        path_prefix = self.sanitize_path_for_filename(
                            str(relative_path.parent)
                        )
                        file_stem_sanitized = self.sanitize_path_for_filename(
                            doc_path.stem
                        )
                        vector_filename_base = f"{path_prefix}_{file_stem_sanitized}"
                    else:  # If it's in the root of NER_RULES_DIR
                        vector_filename_base = self.sanitize_path_for_filename(
                            doc_path.stem
                        )
...
def main():
    ...
    vectorizer = ConstitutionVectorizer(
        rules_dir=str(NER_RULES_DIR_TO_USE), output_dir=VECTORS_OUTPUT_DIR
    )
