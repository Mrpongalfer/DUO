...
def check_python_version():
    if sys.version_info < (
        3,
        9,
    ):  # Relaxed requirement for wider initial use, 3.11 still recommended
        print(
            "WARNING: You are using Python version {}.{}.{}. Project Scribe recommends Python 3.11 or higher for full compatibility.".format(
...
class ScribeConfig:
    ...
    def __init__(
        ...
        if config_path_override:
            ...
            if p_override.is_file():
                self._config_path = p_override
            else:
                self._logger.warning(
                    f"Override config file '{p_override}' not found. Searching defaults."
                )

        if (
            not self._config_path and project_dir_for_local_config
        ):  # Check project dir if override failed or not given
            proj_config = (
                project_dir_for_local_config.resolve() / DEFAULT_CONFIG_FILENAME
            )
            if proj_config.is_file():
                self._config_path = proj_config
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        allowed_bases = (
            ...
            else [str(Path.home()), "/tmp", str(Path.cwd().resolve())]
        )
        self._logger.info(
            f"'allowed_target_bases' default: {allowed_bases} (NAI context: {self._is_nai_context})"
        )
        return {
        ...
        if self._config_path and self._config_path.is_file():
            try:
                with open(self._config_path, "rb") as f:
                    loaded_from_file = tomllib.load(f)
                ...
                self._logger.error(
                    f"Error with config '{self._config_path}': {e}. Using defaults."
                )
        else:
            self._logger.info("No .scribe.toml. Using defaults.")
        try:
            self._validate_loaded_config(config_data)
            self._logger.debug("Config validated.")
        except ScribeConfigurationError as e:
            self._logger.critical(f"Config validation failed: {e}")
            raise
        return config_data

    def _merge_configs(
        self, base: Dict, updates: Dict
    ) -> Dict:  # (Your _merge_configs method)
        m = base.copy()
        for k, v in updates.items():
            if isinstance(v, dict) and isinstance(m.get(k), dict):
                m[k] = self._merge_configs(m[k], v)
            else:
                m[k] = v
        return m
    ...
    ):  # (Your _validate_loaded_config method)
        self._logger.debug(
            "Validating config..."
        )  # Keep your full robust validation logic here
        if not isinstance(config.get("allowed_target_bases"), list):
            raise ScribeConfigurationError("allowed_target_bases: list expected")
    ...
    def get_list(self, key: str, default: Optional[List[Any]] = None) -> List[Any]:
        v = self._config.get(key, default or [])
    ...
    def get_bool(self, key: str, default: bool = False) -> bool:
        ...
        return (
            ...
            else (str(value).lower() == "true" if isinstance(value, str) else default)
        )
    ...
    def get_float(self, key: str, default: float = 0.0) -> float:
        ...
        try:
            ...
            if hasattr(self, "_logger") and self._logger:
                self._logger.warning(
                    f"Could not parse config value for '{key}' ('{value}') as float. Using default: {default}."
                )
            else:  # Fallback print if logger isn't ready (shouldn't happen if config is used after full init)
                print(
                    f"ScribeConfig WARNING: Could not parse config value for '{key}' ('{value}') as float. Using default: {default}.",
                    file=sys.stderr,
                )
            return default
    ...
    def get_int(self, key: str, default: int = 0) -> int:
        ...
        try:
            return int(
                float(value)
            )  # Allow float string to be parsed as int after conversion
        except (ValueError, TypeError):
            if hasattr(self, "_logger") and self._logger:
                self._logger.warning(
                    f"Could not parse config value for '{key}' ('{value}') as int. Using default: {default}."
                )
            else:
                print(
                    f"ScribeConfig WARNING: Could not parse config value for '{key}' ('{value}') as int. Using default: {default}.",
                    file=sys.stderr,
                )
            return default
...
class ToolRunner:
    ...
    def _find_executable(
        self, executable_name: str, venv_path: Optional[Path] = None
    ) -> Path:
        configured_tool_paths = self._config.get("tool_paths", {})
        if (
            isinstance(configured_tool_paths, dict)
            and executable_name in configured_tool_paths
        ):
            ...
            if isinstance(custom_path_str, str) and custom_path_str.strip():
                ...
                if custom_path.is_file() and os.access(custom_path, os.X_OK):
                    self._logger.debug(
                        f"Using configured path for '{executable_name}': {custom_path}"
                    )
                    return custom_path.resolve()
                else:
                    self._logger.warning(
                        f"Configured path for '{executable_name}' ('{custom_path_str}') not executable. Falling back."
                    )
        if venv_path:
            resolved_venv_path = venv_path.resolve()
            if resolved_venv_path.is_dir():
                bin_dir = resolved_venv_path / (
                    "Scripts" if sys.platform == "win32" else "bin"
                )
                for suffix in ["", ".exe"] if sys.platform == "win32" else [""]:
                    venv_exec_candidate = bin_dir / f"{executable_name}{suffix}"
                    if venv_exec_candidate.is_file() and os.access(
                        venv_exec_candidate, os.X_OK
                    ):
                        self._logger.debug(
                            f"Found '{executable_name}' in venv: {venv_exec_candidate}"
                        )
                        return venv_exec_candidate.resolve()
        system_exec_path_str = shutil.which(executable_name)
        if system_exec_path_str:
            self._logger.debug(
                f"Found '{executable_name}' in PATH: {system_exec_path_str}"
            )
    ...
    def run_tool(
        ...
        if not command_args:
            self._logger.critical("run_tool called with empty command_args.")
            return subprocess.CompletedProcess(
                args=[], returncode=127, stdout="", stderr="No command."
            )
        exec_name = command_args[0]
        try:
            exec_path = self._find_executable(exec_name, venv_path)
        except FileNotFoundError as e:
            raise ScribeToolError(f"Executable '{exec_name}' not found.") from e
        full_cmd = [str(exec_path)] + command_args[1:]
        cmd_str = " ".join(shlex.quote(str(a)) for a in full_cmd)
        self._logger.info(f"Running: {cmd_str} (in CWD: {cwd})")
        proc_env = os.environ.copy()
        if venv_path:
            res_venv_path = venv_path.resolve()
            proc_env["VIRTUAL_ENV"] = str(res_venv_path)
        ...
        eff_timeout = (
            ...
            else self._config.get_float("default_tool_timeout", DEFAULT_TOOL_TIMEOUT)
        )
        if eff_timeout <= 0:
            eff_timeout = None
        try:
            ...
            self._logger.debug(
                f"Tool '{exec_name}' RC={result.returncode}. Timeout={eff_timeout or 'None'}s."
            )
            return result
            ...
            return subprocess.CompletedProcess(
                args=e.cmd or full_cmd, returncode=-1, stdout=stdout, stderr=stderr
            )
        except Exception as e:
            raise ScribeToolError(f"OS error executing '{exec_name}': {e}") from e
...
class EnvironmentManager:
    def __init__(
        ...
        self._config = config
        self._venv_path = self._target_dir / VENV_DIR_NAME
        self._venv_python_path = None
        self._pip_path = None
    ...
    def setup_venv(self):  # (Your version)
        self._logger.info(f"Setup venv: {self._venv_path}")
        timeout = self._config.get_float("default_tool_timeout", DEFAULT_TOOL_TIMEOUT)
        if not self._venv_path.exists():
            self._logger.info(f"Creating venv '{self._venv_path}'...")
            py_exe = sys.executable
            try:
                res = self._tool_runner.run_tool(
                    [py_exe, "-m", "venv", str(self._venv_path)],
                    cwd=self._target_dir,
                    timeout=timeout,
                )
                if res.returncode != 0:
                    raise ScribeEnvironmentError(
                        f"Venv create fail (RC={res.returncode}). E: {res.stderr.strip()}"
                    )
                self._logger.info("Venv created.")
            except ScribeToolError as e:
                raise ScribeEnvironmentError(f"Venv create cmd error: {e}") from e
        else:
            self._logger.info("Existing venv found.")
        ...
        if not self._venv_python_path or not self._pip_path:
            raise ScribeEnvironmentError(f"Python/pip not in venv: {self._venv_path}")
        try:
            self._logger.info("Upgrading pip tools...")
            cmd = [
                str(self._pip_path),
                "install",
            ...
            res = self._tool_runner.run_tool(
                cmd, cwd=self._target_dir, venv_path=self._venv_path, timeout=timeout
            )
            if res.returncode != 0:
                self._logger.warning(
                    f"Pip tools upgrade fail (RC={res.returncode}). E: {res.stderr.strip()}"
                )
            else:
                self._logger.info("Pip tools upgraded.")
        except ScribeToolError as e:
            self._logger.warning(f"Error upgrading pip tools: {e}")

    def install_dependencies(self):  # (Your version)
        if not self._pip_path:
            raise ScribeEnvironmentError("Pip in venv not set for dep install.")
        self._logger.info("Installing deps...")
        timeout = self._config.get_float("default_tool_timeout", DEFAULT_TOOL_TIMEOUT)
        pyproj = self._target_dir / "pyproject.toml"
        reqs = self._target_dir / "requirements.txt"
        inst = False
        if pyproj.is_file():
            self._logger.info("Found pyproject.toml.")
            groups = [
            ...
            for grp in groups:
                cmd = base_cmd + [f".{grp}"]
                self._logger.debug(
                    f"Try pip install: {' '.join(shlex.quote(s) for s in cmd)}"
                )
                try:
                    res = self._tool_runner.run_tool(
                        cmd, self._target_dir, self._venv_path, timeout=timeout
                    )
                    if res.returncode == 0:
                        inst = True
                        self._logger.info(
                            f"Installed from pyproject.toml group '{grp}'."
                        )
                        break
                    else:
                        self._logger.warning(
                            f"Pyproject install group '{grp}' fail (RC={res.returncode}). E: {res.stderr.strip()[:200]}..."
                        )
                except ScribeToolError as e:
                    self._logger.warning(f"Tool error pyproject group '{grp}': {e}")
            if not inst:
                raise ScribeEnvironmentError(
                    "All pyproject.toml install attempts failed."
                )
        elif reqs.is_file():
            self._logger.info("Found requirements.txt.")
            cmd = [
                str(self._pip_path),
                "install",
                "--disable-pip-version-check",
            ...
            try:
                ...
                if res.returncode != 0:
                    raise ScribeEnvironmentError(
                        f"requirements.txt install fail (RC={res.returncode}). E: {res.stderr.strip()}"
                    )
                inst = True
                self._logger.info("Installed from requirements.txt.")
            except ScribeToolError as e:
                raise ScribeEnvironmentError(f"Tool error requirements.txt: {e}") from e
        else:
            self._logger.info(
                "No pyproject.toml or requirements.txt. Skipping dep install."
            )
        if not inst and (pyproj.is_file() or reqs.is_file()):
            raise ScribeEnvironmentError("Dep install fail.")

    def run_pip_audit(self) -> subprocess.CompletedProcess:  # (Your version)
        if not self._pip_path:
            raise ScribeEnvironmentError("Pip in venv not set for pip-audit.")
        self._logger.info("Running pip-audit...")
        timeout = self._config.get_float("default_tool_timeout", DEFAULT_TOOL_TIMEOUT)
        inst_cmd = [str(self._pip_path), "install", "pip-audit"]
        try:
            self._logger.debug("Ensuring pip-audit...")
            res = self._tool_runner.run_tool(
                inst_cmd, self._target_dir, self._venv_path, timeout=timeout
            )
            if res.returncode != 0:
                self._logger.warning(
                    f"Pip-audit install fail (RC={res.returncode}). Audit may fail."
                )
            else:
                self._logger.info("Pip-audit available.")
        except ScribeToolError as e:
            self._logger.warning(f"Error installing pip-audit: {e}. Audit may fail.")
        audit_cmd = [
            str(self._pip_path),
            "audit",
            "--format",
        ...
        return self._tool_runner.run_tool(
            audit_cmd, self._target_dir, self._venv_path, timeout=timeout
        )
...
class LLMClient:
    def __init__(self, config: ScribeConfig, cli_args: argparse.Namespace):
        ...
        self._config = config
        self._base_url = cli_args.ollama_base_url or config.get_str(
            "ollama_base_url", DEFAULT_OLLAMA_BASE_URL
        )
        self._model = cli_args.ollama_model or config.get_str(
            "ollama_model", DEFAULT_OLLAMA_MODEL
        )
        self._timeout = config.get_float("ollama_request_timeout", 180.0)
        self._retries = config.get_int("ollama_api_retries", 2)
        self._retry_delay = config.get_float("ollama_api_retry_delay", 5.0)
        ...
        if HTTP_LIB == "httpx":
            try:
                ...
                self._logger.info(
                    f"LLMClient(httpx): URL='{self._base_url}', Model='{self._model}'"
                )
            except Exception as e:
                self._logger.error(f"Httpx client init fail: {e}", exc_info=True)
        elif HTTP_LIB == "requests":
            self._logger.info(
                f"LLMClient(requests): URL='{self._base_url}', Model='{self._model}'"
            )
        else:
            self._logger.error("CRITICAL: No HTTP lib. LLM features unavailable.")

    def _call_api(
        self, prompt: str, fmt: Optional[str] = None
    ) -> Dict[str, Any]:  # (Your version with retries)
        if not HTTP_LIB_AVAILABLE:
            raise ScribeApiError("No HTTP lib for Ollama.")
        ...
        if fmt:
            payload["format"] = fmt
        self._logger.info(
            f"Call Ollama: Model='{self._model}', Format='{fmt or 'text'}' (~{len(prompt)} chars)"
        )
        for attempt in range(self._retries + 1):
            try:
                if self._http_client:
                    resp = self._http_client.post(api_path, json=payload)
                elif HTTP_LIB == "requests":
                    resp = requests.post(
                        f"{self._base_url.rstrip('/')}{api_path}",
                        json=payload,
                        timeout=self._timeout,
                    )
                else:
                    raise ScribeApiError("No HTTP client initialized.")
                resp.raise_for_status()
                json_resp = resp.json()
                if json_resp.get("error"):
                    raise ScribeApiError(f"Ollama API Error: {json_resp['error']}")
                if "response" not in json_resp:
                    raise ScribeApiError("Ollama response missing 'response'.")
                self._logger.info(f"Ollama call success (attempt {attempt+1}).")
                return json_resp
            except (httpx.RequestError, requests.exceptions.RequestException) as e:
                last_exc = e
                self._logger.warning(
                    f"Ollama attempt {attempt+1} fail (Network/Timeout): {e}"
                )
            except json.JSONDecodeError as e:
                last_exc = e
                self._logger.error(f"Ollama JSON decode error: {e}")
                break
            except Exception as e:
                last_exc = e
                self._logger.error(f"Unexpected Ollama API error: {e}", exc_info=True)
                break
            if attempt < self._retries:
                time.sleep(self._retry_delay)
        raise ScribeApiError(
            f"Ollama call failed definitively: {last_exc}"
        ) from last_exc
    ...
    def _extract_code_from_response(self, llm_resp: str) -> str:  # (Your version)
        ...
        if py_blocks:
            if len(py_blocks) > 1:
                self._logger.warning(
                    f"LLM gave {len(py_blocks)} python blocks, concatenating."
                )
            return "\n\n# Scribe: Concatenated Block\n\n".join(
                b.strip() for b in py_blocks
            ).strip()
        gen_blocks = re.findall(
            r"```(?:[a-zA-Z0-9_.-]*)?\n(.*?)\n```", llm_resp, re.DOTALL
        )
        if gen_blocks:
            self._logger.warning("No 'python' block, using first generic.")
            return gen_blocks[0].strip()
        self._logger.warning("No fenced block, assuming all is code.")
        return llm_resp.strip()

    def generate_tests(
        self, code: str, file_name: str, sigs: str
    ) -> Optional[str]:  # (Your version with updated prompt from ScribeConfig)
        self._logger.info(f"Requesting AI test gen for '{file_name}'")
        template = self._config.get_str("test_generation_prompt_template")
        code = code[:7000] + ("..." if len(code) > 7000 else "")
        sigs = sigs[:1000] + ("..." if len(sigs) > 1000 else "")
        try:
            prompt = template.format(
                code_content=code, target_file_path=file_name, signatures=sigs
            )
        except KeyError as e:
            raise ScribeConfigurationError(f"Invalid test_gen_prompt: missing {e}")
        try:
            api_resp = self._call_api(prompt)
            raw_text = api_resp.get("response", "")
            if not raw_text.strip():
                self._logger.warning("LLM empty test response.")
                return None
            extracted = self._extract_code_from_response(raw_text)
            if not extracted:
                self._logger.warning("Failed to extract test code.")
                return None
            ast.parse(extracted)
            self._logger.info("Test code syntax OK.")
            return extracted
        except SyntaxError as e:
            self._logger.error(f"Generated test code syntax error: {e}")
            return None
        except ScribeApiError as e:
            self._logger.error(f"API error test gen: {e}")
            return None
        except Exception as e:
            self._logger.error(f"Unexpected error test gen: {e}", exc_info=True)
            return None
    ...
    def generate_review(
        ...
        List[Dict[str, str]]
    ]:  # (Your version with updated prompt from ScribeConfig)
        ...
        try:
            prompt = template.format(code_content=code, target_file_path=file_name)
        except KeyError as e:
            raise ScribeConfigurationError(f"Invalid review_prompt: missing {e}")
        try:
            api_resp = self._call_api(prompt, output_format_type="json")
            raw_json_str = api_resp.get("response", "")
            if not raw_json_str.strip():
                self._logger.warning("LLM empty review response.")
                return []
            findings = json.loads(raw_json_str)
            if not isinstance(findings, list):
                if isinstance(findings, dict) and all(
                    k in findings for k in ["severity", "description", "location"]
                ):
                    self._logger.warning("LLM review single dict, wrapping.")
                    findings = [findings]
                else:
                    self._logger.error(f"LLM review not list: {type(findings)}")
                    return None
            valid = []
            for item in findings:
                if isinstance(item, dict) and all(
                    k in item for k in ["severity", "description", "location"]
                ):
                    valid.append(
                        {
                            k: str(item[k]).strip()
                            for k in ["severity", "description", "location"]
                        }
                    )
                else:
                    self._logger.warning(f"Skipping malformed review finding: {item}")
            self._logger.info(f"Parsed {len(valid)} review findings.")
            return valid
        except json.JSONDecodeError as e:
            self._logger.error(f"LLM review JSON decode error: {e}")
            return None
        except ScribeApiError as e:
            self._logger.error(f"API error review: {e}")
            return None
        except Exception as e:
            self._logger.error(f"Unexpected error review gen: {e}", exc_info=True)
            return None
...
class WorkflowSteps:
    def __init__(self, agent: "ScribeAgent"):
        self.agent = agent
        self._logger = logging.getLogger(f"{APP_NAME}.WorkflowSteps")

    def _truncate_output(
        self, s: Optional[str], ml: int = 500, hl: int = 200, tl: int = 200
    ) -> str:  # (Your version)
        if not s:
            return ""
            s = s.strip()
    ...
    def validate_inputs(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        ...
        try:
            self.agent._source_file_path = Path(self.agent._args.source_file).resolve(
                strict=True
            )
            rel_target = Path(self.agent._args.target_file)
            if rel_target.is_absolute() or ".." in rel_target.parts:
                raise ScribeInputError("Target file must be relative.")
            self.agent._target_file_path = (
                self.agent._target_dir / rel_target
            ).resolve()
            if not str(self.agent._target_file_path).startswith(
                str(self.agent._target_dir.resolve()) + os.sep
            ):
                raise ScribeInputError("Target file escapes target dir.")
            self.agent._target_file_path.parent.mkdir(parents=True, exist_ok=True)
            if not self.agent._source_file_path.is_file():
                raise ScribeInputError(
                    f"Source not file: {self.agent._source_file_path}"
                )
            details["message"] = (
                f"Inputs OK. Target: {self.agent._target_file_path}, Source: {self.agent._source_file_path}"
            )
            return STATUS_SUCCESS, details
        except Exception as e:
            raise ScribeInputError(f"Input validation error: {e}") from e

    def setup_environment(self) -> Tuple[str, StepOutputDetails]:
        self.agent._env_manager.setup_venv()
        return STATUS_SUCCESS, {
            "message": f"Venv at {self.agent._env_manager.venv_path}"
        }

    def install_deps(self) -> Tuple[str, StepOutputDetails]:
        self.agent._env_manager.install_dependencies()
        return STATUS_SUCCESS, {"message": "Deps installed or skipped."}

    def audit_deps(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        res = self.agent._env_manager.run_pip_audit()
        details: StepOutputDetails = {
            "tool_name": "pip-audit",
        ...
        if res.stdout:
            try:
                vulns = json.loads(res.stdout).get("vulnerabilities", [])
            except json.JSONDecodeError:
                return STATUS_WARNING, {
                    **details,
                    "message": "pip-audit JSON parse error.",
                }
        details["vulnerability_count"] = len(vulns)
        self.agent._report_data["audit_findings"] = vulns
        if not vulns:
            details["message"] = "No vulnerabilities."
            return STATUS_SUCCESS, details
        # (Your severity check logic should be here)
        details["message"] = f"Found {len(vulns)} vulnerabilities."
        return STATUS_WARNING, details  # Simplified

    def apply_code(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        code = self.agent._source_file_path.read_text(encoding="utf-8")
        self.agent._target_file_path.write_text(code, encoding="utf-8")
        return STATUS_SUCCESS, {"bytes_written": len(code)}

    def format_code(
        self,
    ) -> Tuple[str, StepOutputDetails]:  # (Your version with --show-source removed)
        cmd = ["ruff", "format", str(self.agent._target_file_path)]
        res = self.agent._tool_runner.run_tool(
            cmd, self.agent._target_dir, self.agent._env_manager.venv_path
        )
        details = {"tool_name": "ruff (format)", "return_code": res.returncode}
        if res.returncode == 0:
            details["message"] = "Formatted."
            return STATUS_SUCCESS, details
        raise ScribeToolError(
            f"Ruff format fail RC={res.returncode}. E: {res.stderr.strip()}"
        )

    def lint_code(
        self,
    ) -> Tuple[str, StepOutputDetails]:  # (Your version with --show-source removed)
        cmd = ["ruff", "check", "--fix", str(self.agent._target_file_path)]
        res = self.agent._tool_runner.run_tool(
            cmd, self.agent._target_dir, self.agent._env_manager.venv_path
        )
        details = {
            ...
            "return_code": res.returncode,
            "stdout_summary": self._truncate_output(res.stdout),
        }
        if res.returncode == 0:
            details["message"] = "Lint OK."
            return STATUS_SUCCESS, details
        if res.returncode == 1:
            details["message"] = "Lint issues found."
            return (
                STATUS_FAILURE
                if self.agent._config.get_bool("fail_on_lint_critical")
                else STATUS_WARNING
            ), details
        raise ScribeToolError(
            f"Ruff check fail RC={res.returncode}. E: {res.stderr.strip()}"
        )

    def type_check(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        cmd = ["mypy", str(self.agent._target_file_path)]
        res = self.agent._tool_runner.run_tool(
            cmd, self.agent._target_dir, self.agent._env_manager.venv_path
        )
        details = {
        ...
        if res.returncode == 0:
            details["message"] = "MyPy OK."
            return STATUS_SUCCESS, details
        if res.returncode == 1:
            details["message"] = "MyPy type errors."
            return (
                STATUS_FAILURE
                if self.agent._config.get_bool("fail_on_mypy_error")
                else STATUS_WARNING
            ), details
    ...
    def extract_signatures(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                sig_parts = [ast.unparse(d) for d in node.decorator_list]
                sig_parts.append(
                    f"{'async ' if isinstance(node,ast.AsyncFunctionDef) else ''}def {node.name}({ast.unparse(node.args)}){' -> '+ast.unparse(node.returns) if node.returns else ''}:"
                )
                sigs.append(
                    "\n".join(
                        f"@{p}" if i < len(node.decorator_list) else p
                ...
                sig_parts.append(f"class {node.name}{bases}:")
                sigs.append(
                    "\n".join(
                        f"@{p}" if i < len(node.decorator_list) else p
                        for i, p in enumerate(sig_parts)
        ...
        return STATUS_SUCCESS, {
            "message": f"Extracted {len(sigs)} sigs.",
            "raw_output": "\n\n".join(sigs),
        }

    def generate_tests(
        self, sig_details: Optional[StepOutputDetails] = None
    ) -> Tuple[str, StepOutputDetails]:  # (Your version)
        code = self.agent._target_file_path.read_text(encoding="utf-8")
        sigs = sig_details.get("raw_output", "") if sig_details else ""
        tests = self.agent._llm_client.generate_tests(
            code, self.agent._target_file_path.name, sigs
        )
        return (STATUS_SUCCESS if tests else STATUS_FAILURE), {
            "raw_output": tests,
            "message": "Tests AI-generated." if tests else "Test gen failed.",
        }

    def save_tests(
        self, gen_tests_details: Optional[StepOutputDetails] = None
    ) -> Tuple[str, StepOutputDetails]:  # (Your version)
        code = gen_tests_details.get("raw_output", "") if gen_tests_details else ""
        if not code:
            return STATUS_WARNING, {"message": "No test code to save."}
        test_dir = self.agent._target_dir / SCRIBE_TEST_DIR
        test_dir.mkdir(parents=True, exist_ok=True)
        safe_stem = "".join(
            c if c.isalnum() else "_" for c in self.agent._target_file_path.stem
        )
        path = test_dir / f"test_{safe_stem}_scribe.py"
        path.write_text(code, encoding="utf-8")
        return STATUS_SUCCESS, {
            "generated_content_path": str(path),
            "raw_output": str(path),
        }

    def execute_tests(
        self, saved_tests_details: Optional[StepOutputDetails] = None
    ) -> Tuple[str, StepOutputDetails]:  # (Your version)
        path_str = (
            saved_tests_details.get("raw_output", "") if saved_tests_details else ""
        )
        if not path_str or not Path(path_str).is_file():
            return STATUS_FAILURE, {"message": f"Test file not found: {path_str}"}
        # Update PYTHONPATH for pytest
        env = os.environ.copy()
        proj_root_str = str(self.agent._target_dir.resolve())
        src_dir_str = str(
            (self.agent._target_dir / "src").resolve()
        )  # Assuming tests might need to import from 'src'
        ...
        cmd = ["pytest", path_str, "-v"]
        res = self.agent._tool_runner.run_tool(
            cmd, self.agent._target_dir, self.agent._env_manager.venv_path, env_vars=env
        )
        ...
        if res.returncode == 1:
            ...
            return (
                STATUS_FAILURE
                if self.agent._config.get_bool("fail_on_test_failure")
                else STATUS_WARNING
            ), {"message": msg, "stdout_summary": self._truncate_output(res.stdout)}
        if res.returncode == 5:
            msg = "No tests collected by pytest."
            return STATUS_WARNING, {
                "message": msg,
                "stdout_summary": self._truncate_output(res.stdout),
            }
        raise ScribeToolError(
            f"Pytest error RC={res.returncode}. E: {res.stderr.strip()}"
        )

    def review_code(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        code = self.agent._target_file_path.read_text(encoding="utf-8")
        review = self.agent._llm_client.generate_review(
            code, self.agent._target_file_path.name
        )
        self.agent._report_data["ai_review_findings"] = review
        return STATUS_ADVISORY, {
            "issues_found": review or [],
            "message": f"AI review found {len(review or [])} items.",
        }
    ...
    def run_precommit(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        ...
        if not cfg_file.is_file():
            return STATUS_SKIPPED, {"message": "No .pre-commit-config.yaml"}
        cmd = ["pre-commit", "run", "--files", str(self.agent._target_file_path)]
        res = self.agent._tool_runner.run_tool(
            cmd, self.agent._target_dir, self.agent._env_manager.venv_path
        )
        if res.returncode == 0:
            return STATUS_SUCCESS, {"message": "Pre-commit passed."}
        return STATUS_FAILURE, {
            "message": f"Pre-commit failed RC={res.returncode}",
            "stdout_summary": self._truncate_output(res.stdout),
        }

    def commit_changes(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        if not self.agent.git_exe:
            return STATUS_WARNING, {"message": "Git not found, commit skipped."}
        # (Your full git status, add, commit, rev-parse logic)
        status_res = self.agent._tool_runner.run_tool(
            [
                self.agent.git_exe,
                "status",
                "--porcelain",
                str(self.agent._target_file_path),
            ],
            self.agent._target_dir,
        )
        if not status_res.stdout.strip():
            self.agent._report_data["commit_sha"] = "NO_CHANGES"
            return STATUS_SUCCESS, {"message": "No changes to commit."}
        add_res = self.agent._tool_runner.run_tool(
            [self.agent.git_exe, "add", str(self.agent._target_file_path)],
            self.agent._target_dir,
        )
        if add_res.returncode != 0:
            raise ScribeToolError(f"Git add failed: {add_res.stderr}")
        template = self.agent._config.get_str("commit_message_template")
        msg = template.format(target_file=self.agent._target_file_path.name)
        commit_res = self.agent._tool_runner.run_tool(
            [self.agent.git_exe, "commit", "-m", msg], self.agent._target_dir
        )
        if commit_res.returncode != 0:
            if (
                "nothing to commit" in commit_res.stdout.lower()
                or "nothing to commit" in commit_res.stderr.lower()
            ):
                self.agent._report_data["commit_sha"] = "NO_EFFECTIVE_CHANGES_STAGED"
                return STATUS_SUCCESS, {
                    "message": "No effective changes staged for commit."
                }
            raise ScribeToolError(
                f"Git commit failed (RC={commit_res.returncode}): {commit_res.stderr}"
            )
        sha_res = self.agent._tool_runner.run_tool(
            [self.agent.git_exe, "rev-parse", "HEAD"], self.agent._target_dir
        )
        commit_sha = sha_res.stdout.strip()
        self.agent._report_data["commit_sha"] = commit_sha
        return STATUS_SUCCESS, {
            "commit_sha": commit_sha,
            "message": f"Committed SHA: {commit_sha}",
        }
    ...
    def generate_report(self) -> Tuple[str, StepOutputDetails]:  # (Your version)
        ...
        return STATUS_SUCCESS, {
            "message": "Final report data prepared.",
            "raw_output": "Report generation triggered.",
        }
...
class ScribeAgent:
    ...
    def __init__(self):
        ...
        if self._review_only:
            ...
            self._do_commit = (
                self._args.commit
            )  # Use the resolved intent from _parse_arguments
        ...
        self._logger.info(
            f"Scribe Agent {APP_VERSION} initializing. Run ID: {self._run_id}"
        )
        self._logger.debug(
            f"Parsed arguments: {vars(self._args)}"
        )  # vars() for namespace
        ...
        try:
            ...
            (
                logging.getLogger(APP_NAME) if hasattr(self, "_logger") else logging
            ).critical(f"Scribe Agent initialization ScribeError: {e}", exc_info=True)
            self._print_minimal_error_report(
                f"Initialization ScribeError: {e}",
            ...
            self._print_minimal_error_report(
                f"Unexpected Initialization Error: {e}", include_traceback=True
            )
            sys.exit(2)

    def _print_minimal_error_report(
        self,
        error_msg: str,
        ...
        if (
            hasattr(self, "_report_data")
            and self._report_data
            and self._report_data.get("start_time")
        ):
            start_time_iso = self._report_data["start_time"]
        report: Dict[str, Any] = {
            ...
            "target_project_dir": (
                str(self._target_dir) if hasattr(self, "_target_dir") else "Unset"
            ),
            "target_file_relative": (
                self._args.target_file if hasattr(self, "_args") else "Unset"
            ),
            "language": "python",
            "python_version": sys.version.split()[0],
            "commit_attempted": self._do_commit,
            "commit_sha": None,
            "steps": [
                {
        ...
        if error_details:
            report["steps"][0]["details"] = error_details
        if include_traceback:
            report["steps"][0].setdefault("details", {})[
                "traceback"
        ...
        try:
            ...
            print(
                f'{{"error": "Failed to generate minimal error report", "details": "{str(e_print)}"}}'
            )
    ...
    def _parse_arguments(self) -> argparse.Namespace:
        ...
        parser.add_argument(
            "target_dir", help="The target project directory (must be a Git repo)."
        )
        parser.add_argument(
            "--source-file",
            required=True,  # Your script used --code-file, standardized to --source-file
        ...
        parser.add_argument(
            "--log-file", default=None, help="Detailed DEBUG log file path."
        )
        parser.add_argument(
            "--config-file", default=None, help="Custom .scribe.toml config file path."
        )
        parser.add_argument(
            "--ollama-base-url", default=None, help="Override Ollama base URL."
        )
        parser.add_argument(
            "--ollama-model", default=None, help="Override Ollama model."
        )
        parser.add_argument(
            "--nai-context", action="store_true", help="NAI TUI context flag."
        )
        ...
        if parsed_args.commit_explicitly_true:
            ...
            parsed_args.commit_explicitly_false
        ):  # This won't be hit from CLI as it's suppressed
    ...
    def _validate_target_dir(self, dir_path_str: str) -> Path:
        ...
        try:
            ...
            if not target_dir.is_dir():
                raise ScribeInputError(f"Target path '{target_dir}' not a dir.")
            if not hasattr(self, "_config"):
                raise ScribeConfigurationError(
                    "Config not init for target dir validation."
                )
            allowed_bases_cfg = self._config.get_list("allowed_target_bases", [])
            if not allowed_bases_cfg:
                self._logger.warning(
                    "'allowed_target_bases' empty. No base path restriction."
                )
                return target_dir
            allowed_bases_resolved = [
                str(Path(p).expanduser().resolve()) for p in allowed_bases_cfg
            ]
            if not any(str(target_dir).startswith(b) for b in allowed_bases_resolved):
                raise ScribeInputError(
                    f"Target dir '{target_dir}' not in allowed bases: {allowed_bases_resolved}."
                )
            return target_dir
        except FileNotFoundError:
            raise ScribeInputError(f"Target project dir not found: '{dir_path_str}'")
        except Exception as e:
            raise ScribeInputError(f"Error validating target dir '{dir_path_str}': {e}")

    def _find_git_executable(self) -> Optional[str]:
        # (Your existing _find_git_executable method. Ensure it uses self._tool_runner.)
        try:
            return str(self._tool_runner._find_executable("git"))
        except (FileNotFoundError, ScribeToolError):
            self._logger.warning("'git' not found.")
            return None
    ...
    def run(self):
        ...
        self._logger.info(
            f"Target Project: {self._target_dir}"
        )  # Uses validated self._target_dir
        ...
        for step_name in validation_steps:
            if not hasattr(self, "_workflow") or self._workflow is None:
                ...
                self._overall_success = False
                self._add_step_result_custom(
                    step_name, STATUS_FAILURE, {"message": "Workflow not initialized"}
                )
                break
            ...
            if not callable(step_func):
                self._logger.error(
                    f"Configuration Error: Step '{step_name}' not a valid method in WorkflowSteps. Halting."
                )
                self._overall_success = False
                self._add_step_result_custom(
                    step_name,
            ...
            if step_name == "run_precommit" and self._review_only:
                self._logger.info(
                    f"Skipping step '{step_name}' due to --review-only flag."
                )
                self._add_step_result_custom(
                    step_name,
                    STATUS_SKIPPED,
                    {"message": "Skipped: --review-only active."},
                )
                continue
                ...
                if not self._do_commit:
                    skip_reason = (
                        ...
                        if self._review_only
                            ...
                            "--no-commit active"
                            if self._no_commit
                            else "Commit not planned"
                        )
                    )
                    self._logger.info(
                        f"Skipping step '{step_name}' because effective commit is False ({skip_reason})."
                    )
                    self._add_step_result_custom(
                        step_name,
                        STATUS_SKIPPED,
            ...
            if step_name == "generate_tests":
                dependency_arg = step_outputs.get("extract_signatures", {}).get(
                    "details"
                )
            ...
            if not self._execute_step(
                step_name, step_func, step_outputs, dependency_arg
            ):
                self._logger.warning(
                    f"Pipeline halting at step '{step_name}' due to its failure."
                )
            ...
            if step_name == "review_code" and self._review_only:
                ...
                if last_step_status not in [STATUS_FAILURE]:
                    self._logger.info(
                        "Review-only mode: Pipeline finished successfully after 'review_code'."
                    )
                    # _overall_success remains True if all prior steps were successful
                else:
                    self._logger.warning(
                        "Review-only mode: 'review_code' or prior step failed. Pipeline halting."
                    )
                    self._overall_success = (
                        False  # Ensure overall failure if review or prior step failed
                    )
    ...
    def _execute_step(
        ...
        try:
            ...
            if not self._overall_success and name != "generate_report":
                step_result["status"] = STATUS_SKIPPED
                step_result["details"] = {"message": "Skipped due to prior failure."}
                self._logger.warning(f"Skipping step '{name}' due to prior failure.")
                self._add_step_result(step_result)
                return True
            status, details_payload = (
                step_func(dependency) if dependency else step_func()
            )
            step_result["status"] = status
            step_result["details"] = details_payload
            step_outputs[name] = step_result  # Use the full step_result for context
            if status == STATUS_FAILURE:
                self._overall_success = False
        except ScribeError as e:
            self._logger.error(
                f"Step '{name}' controlled ScribeError: {e}", exc_info=False
            )
            self._logger.debug("Traceback:", exc_info=True)
            step_result["status"] = STATUS_FAILURE
            step_result["error_message"] = str(e)
            step_result["details"] = getattr(e, "details", {"message": str(e)})
            self._overall_success = False
        except Exception as e:
            self._logger.critical(f"Step '{name}' UNEXPECTED error: {e}", exc_info=True)
            step_result["status"] = STATUS_FAILURE
            step_result["error_message"] = f"Unexpected: {type(e).__name__} - {e}"
            step_result["details"] = {"traceback": traceback.format_exc()}
            self._overall_success = False
        self._add_step_result(step_result)
        return step_result["status"] != STATUS_FAILURE

    def _add_step_result_custom(
        self,
        name: str,
        status: str,
        ...
        if hasattr(self, "_report_data") and "steps" in self._report_data:
            self._report_data["steps"].append(step_res)
        else:
            self._logger.error(
                f"Cannot add custom step '{name}', _report_data.steps missing."
            )
    ...
    def _add_step_result(self, result: StepResult):
        ...
        result["end_time"] = et.isoformat()
        try:
            st_obj = datetime.fromisoformat(result["start_time"])
            result["duration_seconds"] = round((et - st_obj).total_seconds(), 3)
        except:
            result["duration_seconds"] = -1.0
        if hasattr(self, "_report_data") and "steps" in self._report_data:
            self._report_data["steps"].append(result)
        else:
            self._logger.error(
                f"Cannot add step result for '{result['name']}', _report_data.steps missing."
            )
        self._logger.info(
            f"--- Step '{result['name']}' Finished | Status: {result['status']} | Duration: {result['duration_seconds']:.3f}s ---"
        )

    def _generate_and_print_final_report(self):
        # (Your existing _generate_and_print_final_report method from turn 35)
        if not hasattr(self, "_report_data") or not self._report_data:
            self._print_minimal_error_report("Report data missing.")
            return
        self._report_data["end_time"] = datetime.now(timezone.utc).isoformat()
        if self._report_data.get("start_time"):
            try:
                st = datetime.fromisoformat(self._report_data["start_time"])
                et = datetime.fromisoformat(self._report_data["end_time"])
                self._report_data["total_duration_seconds"] = round(
                    (et - st).total_seconds(), 3
                )
            except:
                self._report_data["total_duration_seconds"] = -1.0
        self._report_data["overall_status"] = (
            STATUS_SUCCESS if self._overall_success else STATUS_FAILURE
        )
        commit_step_cfg = "commit_changes" in self._config.get_list(
            "validation_steps", []
        )
        self._report_data["commit_attempted"] = (
            self._do_commit and commit_step_cfg
        )  # Correctly reflects if commit was planned and configured
        if hasattr(self, "_report_generator") and self._report_generator:
            print(self._report_generator.generate(self._report_data))
        else:
            print(json.dumps(self._report_data, indent=2, default=str))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--review-only",
        action="store_true",
        help="Perform review without committing changes.",
    )
    parser.add_argument("--no-commit", action="store_true", help="Skip commit step.")
    parser.add_argument(
        "--log-level", default="INFO", choices=LOG_LEVELS, help="Set log level."
    )
