# Migrated from ODA/oda_core_operative_blueprint.py
# Unified ODA core operative blueprint
...
class ProjectScaffolder:
    ...
    def __init__(
        ...
        if template_source_type != "local":
            ...
            self.im.present_information(
                "Only 'local' template source type supported in MVP.", style="error"
            )
            raise typer.Exit(1)
    ...
    def _present_final_output(self, project_root: Path, details: Dict[str, Any]):
        ...
        self.console.print(
            "  2. Review the project's [cyan]README.md[/cyan] for specific instructions."
        )
        self.console.print(
            "  3. Activate the virtual environment: [cyan]poetry shell[/cyan]"
        )
        self.console.print(
            "  4. Set required environment variables in [cyan].env[/cyan]."
        )
        self.console.print("  5. Begin developing!")
...
# --- ODA Priming Ritual Implementation ---
class ODAPrimer:
    """
    Generates a primed operational state for ODA from Edict files or templates.
    """

    def __init__(self, edict_dir: Path = Path("./edicts")):
        self.edict_dir = edict_dir
        self.default_state = {
            "oda_version": "0.1-primed",
            "architect_identity": {"name": "Architect", "email": ""},
            "tpc_standards": {
                "default_license": "MIT",
                "require_docker": True,
                "require_ci_cd": True,
            ...
            "interaction_protocols": {"confirm_major_actions": True, "style": "direct"},
            "operational_philosophy": {"default_timeout": 300},
            "template_sources": {"type": "local", "path": "./oda_templates"},
        }

    def generate_state(self) -> dict:
        # Try to load Edict files for persona, standards, etc. If not found, use defaults.
        state = self.default_state.copy()
        try:
            persona_file = self.edict_dir / "00_Persona_Foundation.md"
            if persona_file.exists():
                with open(persona_file, "r", encoding="utf-8") as f:
                    persona_lines = f.readlines()
                # Extract name/email from first lines if present
                for line in persona_lines:
                    if line.lower().startswith("name:"):
                        state["architect_identity"]["name"] = line.split(":", 1)[
                            1
                        ].strip()
                    if line.lower().startswith("email:"):
                        state["architect_identity"]["email"] = line.split(":", 1)[
                            1
                        ].strip()
            # Could add more parsing for other Edicts here
        except Exception as e:
            console.print(
                f"[yellow]Warning:[/yellow] Failed to parse Edict persona: {e}"
            )
        return state

    def save_state(self, state: dict, output_path: Path):
        try:
            with open(output_path, "w", encoding="utf-8") as f:
                json.dump(state, f, indent=2)
            console.print(
                f"[green]✓[/green] Created primed state file at '{output_path}'."
            )
        except Exception as e:
            console.print(f"[red]✗[/red] Failed to create primed state file: {e}")


@oda_app.command(
    "prime", help="Run the Edict Priming Ritual to generate ODA's primed state."
)
def prime_cmd(
    config_output: Path = typer.Option(
        "oda_primed_state.json", help="Path to save the generated primed state file."
    ),
    edict_dir: Path = typer.Option(
        "./Lily/LilyCoreMemory/",
        help="Directory containing Edict persona/standards files.",
        exists=False,
        file_okay=False,
        dir_okay=True,
        readable=True,
    ...
    """Generate a primed operational state for ODA from Edict files or defaults."""
    console.print("[bold yellow]ODA Priming Ritual[/bold yellow]")
    primer = ODAPrimer(edict_dir)
    state = primer.generate_state()
    primer.save_state(state, config_output)


# Import platform at the end if used within functions that need it.

# --- Autonomous Lily Evolution Enhancements ---
import threading
import time


class LogWatcher:
    """Watches a directory for new log entries and triggers learning cycles."""

    def __init__(self, log_dir: Path, callback, poll_interval: float = 5.0):
    ...
    def _watch_loop(self):
        while not self._stop_event.is_set():
            new_files = set(self.log_dir.glob("*.log"))
            added = new_files - self._last_seen
            if added:
                for f in added:
                    self.callback(f)
            self._last_seen = new_files
            time.sleep(self.poll_interval)


class EvolutionProposalEngine:
    """Generates and applies evolution proposals for Lily personas/code."""

    def __init__(self, persona_dir: Path, proposals_dir: Path, im: InteractionManager):
        self.persona_dir = Path(persona_dir)
        self.proposals_dir = Path(proposals_dir)
        self.im = im

    def generate_proposal(self, context_info: dict) -> Path:
        """Analyze context and generate a proposal file."""
        proposal_text = f"# Evolution Proposal\nContext: {context_info}\n- [ ] Review and update persona logic as needed.\n"
        proposal_path = self.proposals_dir / f"proposal_{int(time.time())}.md"
        self.proposals_dir.mkdir(parents=True, exist_ok=True)
        with open(proposal_path, "w", encoding="utf-8") as f:
            f.write(proposal_text)
        self.im.present_information(
            f"Generated evolution proposal: {proposal_path}", style="success"
        )
        return proposal_path

    def apply_proposal(self, proposal_path: Path) -> bool:
        """Apply the proposal to the persona/codebase (MVP: log action)."""
        # In full impl, parse and apply changes. Here, just log.
        self.im.present_information(f"Applying proposal: {proposal_path}", style="info")
        # TODO: Implement actual application logic
        return True


class VersionControlManager:
    """Handles Git and Ansible integration for versioning and deployment."""

    def __init__(self, repo_dir: Path, tools: ToolOrchestrator, im: InteractionManager):
    ...
    def commit_and_push(self, message: str) -> bool:
        if not shutil.which("git"):
            self.im.present_information("Git not found, cannot commit.", style="error")
            return False
        ok, _, _ = self.tools.run_command(
            ["git", "add", "."], cwd=self.repo_dir, desc="Git Add"
        )
        if not ok:
            return False
        ok, _, _ = self.tools.run_command(
            ["git", "commit", "-m", message], cwd=self.repo_dir, desc="Git Commit"
        )
        if not ok:
            return False
        ok, _, _ = self.tools.run_command(
            ["git", "push"], cwd=self.repo_dir, desc="Git Push"
        )
        return ok

    def deploy_with_ansible(self, playbook: str) -> bool:
        if not shutil.which("ansible-playbook"):
            self.im.present_information(
                "Ansible not found, cannot deploy.", style="error"
            )
            return False
        ok, _, _ = self.tools.run_command(
            ["ansible-playbook", playbook], cwd=self.repo_dir, desc="Ansible Deploy"
        )
        return ok


class ValidationManager:
    """Enhanced validation for persona documents and code."""

    def __init__(self, im: InteractionManager):
        self.im = im

    def validate_persona(self, persona_path: Path) -> bool:
        # MVP: Check file exists and is not empty
        if not persona_path.exists() or persona_path.stat().st_size == 0:
            self.im.present_information(
                f"Persona file invalid: {persona_path}", style="error"
            )
            return False
        self.im.present_information(
            f"Persona file valid: {persona_path}", style="success"
        )
        return True

    def validate_code(self, code_path: Path) -> bool:
        # MVP: Run flake8 or similar
        if not shutil.which("flake8"):
            self.im.present_information(
                "flake8 not found, skipping code validation.", style="warning"
            )
            return True
        ok, _, _ = self.im.primed_state.get_param(
            "tpc_standards", "require_code_validation", True
        )
        if not ok:
            return True
        result = os.system(f"flake8 {code_path}")
        if result != 0:
            self.im.present_information(
                f"Code validation failed: {code_path}", style="error"
            )
            return False
        self.im.present_information(
            f"Code validation passed: {code_path}", style="success"
        )
        return True


class OversightManager:
    """Implements safeguards, monitoring, and reporting."""

    def __init__(self, im: InteractionManager, audit_log: Path):
        self.im = im
    ...
    def log_event(self, event: str):
        ...
        self.im.present_information("Audit event logged.", style="info")

    def circuit_breaker(self, condition: bool, reason: str) -> bool:
        if condition:
            self.log_event(f"CIRCUIT BREAKER TRIGGERED: {reason}")
            self.im.present_information(
                f"Circuit breaker triggered: {reason}", style="error"
            )
            return True
        return False


class LilyEvolutionOrchestrator:
    """Main orchestrator for the autonomous Lily evolution cycle."""

    def __init__(
        self,
        im: InteractionManager,
        tools: ToolOrchestrator,
        state: PrimedOperationalState,
    ):
        self.im = im
        self.tools = tools
        self.state = state
        self.log_dir = Path(
            "../../Lily/LilyCoreMemory/InteractionLog_ContextualSnapshots/"
        ).resolve()
        self.persona_dir = Path("../../Lily/LilyCoreMemory/KeyDirectives/").resolve()
        self.proposals_dir = Path(
            "../../Lily/LilyCoreMemory/ProposedPersonaUpdates/"
        ).resolve()
        self.repo_dir = Path("../../Lily/").resolve()
        self.audit_log = Path("lily_evolution_audit.log")
        self.validation = ValidationManager(im)
        self.oversight = OversightManager(im, self.audit_log)
        self.vc = VersionControlManager(self.repo_dir, tools, im)
        self.proposals = EvolutionProposalEngine(
            self.persona_dir, self.proposals_dir, im
        )
        self.log_watcher = LogWatcher(self.log_dir, self._on_new_log)
        self._active = False

    def start(self):
        self.im.present_information(
            "Starting Lily Evolution Orchestrator...", style="info"
        )
        self._active = True
        self.log_watcher.start()
        self.im.present_information(
            "Log watcher started. Awaiting new logs...", style="info"
        )
        try:
            while self._active:
                time.sleep(1)
    ...
    def stop(self):
        ...
        self.im.present_information(
            "Lily Evolution Orchestrator stopped.", style="warning"
        )

    def _on_new_log(self, log_path: Path):
        self.im.present_information(f"Detected new log: {log_path}", style="info")
        # Safeguard: Only process if not in circuit breaker
        if self.oversight.circuit_breaker(
            False, "MVP: always allow"
        ):  # Replace with real checks
            return
        # Validate persona before proposing evolution
        persona_files = list(self.persona_dir.glob("*.md"))
        for pf in persona_files:
            if not self.validation.validate_persona(pf):
                self.oversight.log_event(f"Invalid persona: {pf}")
                return
        # Generate and apply proposal
        proposal = self.proposals.generate_proposal({"log": str(log_path)})
        if not self.proposals.apply_proposal(proposal):
            self.oversight.log_event(f"Failed to apply proposal: {proposal}")
            return
        # Version control
        if not self.vc.commit_and_push(f"Apply evolution proposal: {proposal.name}"):
            self.oversight.log_event("Version control commit/push failed.")
        # Optionally deploy
        # self.vc.deploy_with_ansible("../nexus_omniengine_v3/ansible/agent_update.yml")
        self.oversight.log_event(f"Evolution cycle complete for log: {log_path}")


# --- CLI Command for Autonomous Lily Evolution ---
@oda_app.command("autoevolve", help="Start the autonomous Lily evolution orchestrator.")
def autoevolve_cmd(ctx: typer.Context):
    if not ctx.obj or not ctx.obj.get("state"):
        console.print("[bold red]ODA State not initialized. Exiting.[/bold red]")
        raise typer.Exit(code=3)
    orchestrator = LilyEvolutionOrchestrator(
        im=ctx.obj["im"],
        tools=ctx.obj["tools"],
        state=ctx.obj["state"],
    )
