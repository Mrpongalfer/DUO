...
def display_panel(
    ...
    try:
        ...
        console.print("[bold red]Error rendering panel content.[/bold red]")
...
def display_syntax(
    ...
    if title:
        console.print(
            Panel(syntax, title=title, border_style="blue", expand=False)
        )  # expand=False for code typically
    else:
...
def select_from_list_rich(
    items: List[str], prompt_message: str, default_choice: Optional[str] = None
) -> Optional[str]:
    # ... (rest of this function as previously defined) ...
    if not items:
        return None
    ...
    while True:
        choice_num_str = IntPrompt.ask(
            ...
            default=(
                0
                if default_choice is None or default_choice not in items
                else items.index(default_choice) + 1
            ),
        )
        if choice_num_str == 0:
            return None
...
def fzf_select(
    ...
    try:
        process = subprocess.run(
            fzf_command, input=items_str, text=True, capture_output=True, check=False
        )
        if process.returncode == 0:
            return (
                process.stdout.strip().splitlines() if multi else process.stdout.strip()
            )
            ...
            logger.warning(
                f"fzf exited with unexpected code {process.returncode}. Stderr: {process.stderr.strip()}"
            )
            return [] if multi else None
    except FileNotFoundError:
        logger.error(
            f"fzf command ('{fzf_executable}') not found. Please install fzf or ensure it's in PATH."
        )
        if not multi:
            return select_from_list_rich(items, f"(fzf fallback) {prompt}")
        else:
            console.print(
                "[red]fzf not found, cannot perform multi-selection. Try installing fzf.[/red]"
            )
            return []
    except Exception as e:
...
def get_text_input(
    prompt_message: str, default_value: Optional[str] = None, password: bool = False
) -> str:
    # ... (rest of this function as previously defined) ...
...
def print_agent_output(
    ...
    content_parts = [
        f"[bold {'green' if success else 'red'}]Execution Status: {'SUCCESS' if success else 'FAILURE'}[/bold {'green' if success else 'red'}]"
    ]

    if "error" in output_data:
        content_parts.append(
            f"[bold red]Error Message:[/bold red] {output_data['error']}"
        )
    ...
    if "action_results" in output_data and isinstance(
        output_data["action_results"], list
    ):
        ...
        for i, res in enumerate(output_data["action_results"]):
            ...
            if isinstance(act_msg_payload, dict):
                act_msg_payload = "{...} (details in logs or full JSON output)"  # Correctly escaped for the outer f-string
            elif isinstance(act_msg_payload, str) and len(act_msg_payload) > 150:
                act_msg_payload = act_msg_payload[:150] + "..."
            content_parts.append(
                f"  {i+1}. {act_type}: [{'green' if act_success else 'red'}]Succeeded: {act_success}[/{'green' if act_success else 'red'}] - {act_msg_payload}"
            )
        ...
        for i, step_res in enumerate(output_data["steps"]):
            ...
            step_msg = (
                step_msg_obj.get(
                    "message", step_res.get("error_message", "No message.")
                )
                if isinstance(step_msg_obj, dict)
                else str(step_msg_obj)
            )
            if len(str(step_msg)) > 100:
                step_msg = str(step_msg)[:100] + "..."
            ...
            if step_status == "FAILURE":
                ...
                color = "dim"
            content_parts.append(
                f"  {i+1}. {step_name}: [{color}]{step_status}[/{color}] - {step_msg}"
            )
