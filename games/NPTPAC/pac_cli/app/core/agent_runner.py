...
class BaseAgentRunner:
    ...
    def __init__(
        self, agent_name: str, agent_script_path_str: Optional[str], config_manager: Any
    ):  # config_manager is type 'ConfigManager'
        ...
        if not self.agent_script_path_str:
            logger.error(
                f"Path for {self.agent_name} is not configured. It cannot be run."
            )
            ...
            if not self.agent_script_command:
                logger.error(
                    f"Agent command string '{self.agent_script_path_str}' for {self.agent_name} is empty after shlex.split."
                )
    ...
    def _prepare_env(
        self, project_env_vars: Optional[Dict[str, str]] = None
    ) -> Dict[str, str]:
    ...
    def run(
        ...
        if not self.agent_script_command:
            error_msg = (
                f"{self.agent_name} path not configured or invalid. Cannot execute."
            )
            logger.error(error_msg)
            return (
                False,
                {"error": error_msg, "details": "Agent path missing in PAC config."},
                None,
                None,
            )

        effective_cwd = cwd or (
            self.config_manager.npt_base_dir if self.config_manager else Path.cwd()
        )
        effective_timeout = timeout_seconds or self.config_manager.get(
            f"agents.{self.agent_name.lower()}_timeout", 300
        )  # Default 5 mins
        ...
        try:
            ...
            if stdout_str:
                logger.debug(f"{self.agent_name} STDOUT:\n{stdout_str}")
            if stderr_str:
                logger.debug(f"{self.agent_name} STDERR:\n{stderr_str}")
            ...
            if process.returncode == 0:
                try:
                    ...
                    if (
                        ...
                        and "overall_success" in output_data
                        and not output_data["overall_success"]
                    ):
                        logger.warning(
                            f"{self.agent_name} reported internal failure (overall_success: false) despite RC=0."
                        )
                        return False, output_data, stdout_str, stderr_str
                    return True, output_data, stdout_str, stderr_str
                except json.JSONDecodeError:
                    logger.warning(
                        f"{self.agent_name} STDOUT was not valid JSON. Treating as success with raw output."
                    )
                    return (
                        True,
                        {"raw_output": stdout_str, "warning": "Output not JSON"},
                        stdout_str,
                        stderr_str,
                    )
            else:
                ...
                error_message = (
                    f"Agent '{self.agent_name}' failed with exit code {process.returncode} while performing task.\n"
                    f"--- STDERR ---\n{stderr_str}\n--- STDOUT ---\n{stdout_str}\n"
                    f"Please check the agent script, input parameters, and logs for more details."
                )
                logger.error(error_message)
                # Try to parse JSON error from stdout if present and contractually expected
                error_payload = {
                    "error": error_message,
                ...
                # Only parse JSON if agent is known to emit structured errors
                if stdout_str.strip().startswith("{"):
                    try:
                        error_json = json.loads(stdout_str)
                        error_payload["agent_error_json"] = error_json
                    except Exception:
                        pass
            ...
            return (
                False,
                {"error": "TimeoutExpired", "details": error_msg},
                None,
                error_msg,
            )
            ...
            return (
                False,
                {"error": "AgentScriptNotFound", "details": error_msg},
                None,
                error_msg,
            )
        except Exception as e:
            error_msg = (
                f"Unexpected error running {self.agent_name}: {type(e).__name__} - {e}"
            )
...
class ExWorkAgentRunner(BaseAgentRunner):
    ...
    def execute_instruction_block(
        self,
        instruction_json_str: str,
        project_path: Path,
        timeout_seconds: Optional[int] = None,
    ) -> Tuple[bool, Dict[str, Any]]:
        """Sends a JSON instruction block to Ex-Work via stdin and gets JSON response."""
        if not self.agent_script_command:
            return False, {"error": "Ex-Work agent not configured."}
...
class ScribeAgentRunner(BaseAgentRunner):
    ...
    def run_validation(
        ...
        if not self.agent_script_command:
            return False, {"error": "Scribe agent not configured."}
