...
class NERHandler:
    ...
    def __init__(
        self, ner_root_path: Path, config_manager: Optional[Any] = None
    ):  # config_manager for future use (e.g. NER validation schemas)
        ...
        if not self.ner_root.is_dir():
            ...
            logger.critical(
                f"NER root path does not exist or is not a directory: {self.ner_root}"
            )
    ...
    def list_categories(self) -> List[str]:
        ...
        try:
            return sorted(
                [
                    d.name
                    for d in self.ner_root.iterdir()
    ...
    def list_items_in_category(
        self, category_path_relative: str, recursive: bool = False
    ) -> List[Dict[str, str]]:
        ...
        if not str(category_abs_path).startswith(str(self.ner_root)):
            logger.warning(
                f"Attempt to list items outside NER root rejected: {category_path_relative}"
            )
            return []
        ...
        try:
            ...
            logger.error(
                f"Error listing items in NER category {category_abs_path}: {e}"
            )
        return items
    ...
    def get_item_content(self, item_relative_path_to_ner: str) -> Optional[str]:
        ...
        if not str(item_abs_path).startswith(str(self.ner_root)):
            logger.warning(
                f"Attempt to read item outside NER root rejected: {item_relative_path_to_ner}"
            )
            return None
        ...
        if item_abs_path.is_file():
            try:
                ...
                logger.error(
                    f"Unicode decode error reading NER item {item_abs_path}: {e}"
                )
                return "# ERROR: Could not decode file content (not valid UTF-8)."
        else:
            logger.warning(
                f"Requested NER item is not a file or does not exist: {item_abs_path}"
            )
            return None  # Or raise error

    def search_ner(
        self, query: str, search_in_category: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Searching NER for '{query}'{f' in category {search_in_category}' if search_in_category else ''}..."
        )
        results = []
        search_root = (
            (self.ner_root / search_in_category).resolve()
            if search_in_category
            else self.ner_root
        )

        if (
            not str(search_root).startswith(str(self.ner_root))
                ...
                if item_path.is_file() and not item_path.name.startswith("."):
                    ...
                    if match_found_in_filename or match_found_in_content:
                        results.append(
                            {
                                "name": item_path.name,
                                "relative_path_to_ner": str(
                                    item_path.relative_to(self.ner_root)
                                ),
                                "type": "file",
                                "match_type": (
                                    "filename_and_content"
                                    if match_found_in_filename
                                    and match_found_in_content
                                    else (
                                        "filename"
                                        if match_found_in_filename
                                        else "content"
                                ...
                                "snippet": (
                                    content_snippet if match_found_in_content else ""
                                ),
                            }
                        )
        except OSError as e:
    ...
    def git_commit_ner_changes(
        self, commit_message: str, add_all: bool = True
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
            )  # check=False to parse output

            if result.returncode == 0:
                return (
                    True,
                    f"NER changes committed successfully. Output:\n{result.stdout}",
                )
            elif (
                "nothing to commit" in result.stdout.lower()
                or "no changes added to commit" in result.stdout.lower()
            ):
