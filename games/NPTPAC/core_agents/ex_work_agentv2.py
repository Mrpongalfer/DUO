...
def request_signoff_helper_direct_tty(message: str, step_id: str) -> Tuple[bool, str]:
    ...
    signoff_mode = os.environ.get(
        "EXWORK_SIGNOFF_MODE", "tty"
    ).lower()  # tty, file, auto, webhook
    start_time = time.time()
    ...
    try:
        if signoff_mode == "tty":
            ...
            with open(awaiting_flag, "w") as f:
                f.write(message)
            logger.info(
                f"Signoff awaiting file created: {awaiting_flag}. Waiting for approval file: {approved_flag}"
            )
            sys.stderr.write(
            ...
            while waited < timeout:
                ...
                success = False
                response_str = (
                    f"Timed out waiting for signoff file after {timeout} seconds."
                )
